...
from models.preact_parseval import *
...
trainset = torchvision.datasets.CIFAR10(root='/homes/crosarko/pytorch-cifar/data', train=True, download=True, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=100, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR10(root='/homes/crosarko/pytorch-cifar/data', train=False, download=True, transform=transform_test)
...
def train(epoch, optimizer):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        progress_bar(batch_idx, len(trainloader), 'Log Loss: %.3f | Loss: %.3f | Acc: %.3f%% (%d/%d)'
            % (train_loss1/(batch_idx+1),train_loss/(batch_idx+1), 100.*correct/total, correct, total))
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        progress_bar(batch_idx, len(testloader), 'Log Loss: %.3f | Loss: %.3f | Acc: %.3f%% (%d/%d)'
            % (test_loss1/(batch_idx+1),test_loss/(batch_idx+1), 100.*correct/total, correct, total))
